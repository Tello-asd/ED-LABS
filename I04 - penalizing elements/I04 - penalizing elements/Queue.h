...
public:
   ...
   the element in the position given in the parameter (starting counting on zero) and move to the end of 
   the queue
    **/
	void penalizeElem(int position) {
		
		/*previous is the previous element to the one penalized*/
		Node* previous = _first;
		if (position == 0) {//check if the penalized element is the first one.
		
			Node* penalize = previous;
			Node* nextOne = penalize->_next;
			_first = nextOne;
			penalize->_next = NULL;

			if (_last != NULL) {
				_last->_next = penalize;
			}
			_last = penalize;
		}
		else {
			for (int i = 0; i < position - 1; i++) {//check in the queue the previous penalized
				previous = previous->_next;
				
			}
			
			Node* penalize = previous->_next; //the penalized one will follow the previous one.
		
			if (penalize->_next != NULL) {
			
				Node* nextOne = penalize->_next;
				previous->_next = nextOne;
				
				//we put the pnealized one the last position
				penalize->_next = NULL;
				if (_last != NULL) {
					_last->_next = penalize;
				}
